...
matches="$(
    find \
        "${repo_root}/server/engine/src/base" \
        "${repo_root}/server/engine/src/common" \
        "${repo_root}/server/engine/src/adapters" \
        -type f \
        ...
        ! -path "${repo_root}/server/engine/src/common/id/*" \
        -print0 |
    xargs -0 grep -n -E "${pattern}" || true
)"

if [ -n "${matches}" ]; then
    printf '%s\n' "Direct allocation is only allowed in selected engine ownership modules:"
    printf '%s\n' "${matches}"
    exit 1
fi
