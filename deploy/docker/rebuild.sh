...
# Command description:
#   Clean and rebuild project code inside the default Docker dev container.
...
usage() {
  cat <<'EOF'
Usage:
  deploy/docker/rebuild.sh [target]
...
cmd="$(env_prefix BUILD_DIR JOBS CMAKE_ARGS)./scripts/rebuild.sh"
if [ "$#" -eq 1 ]; then
  cmd="${cmd} $(quote_arg "$1")"
fi
